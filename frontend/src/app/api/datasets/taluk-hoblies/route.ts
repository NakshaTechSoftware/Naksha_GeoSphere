...
const DISTRICTS: Array<[string, string]> = [
  ["01", "Belagavi"], ["02", "Bagalkote"], ["03", "Vijayapura"], ["04", "Kalaburgi"],
  ["05", "Bidar"], ["06", "Raichur"], ["07", "Koppal"], ["08", "Gadag"],
  ["09", "Dharwad"], ["10", "Uttara Kannada"], ["11", "Haveri"], ["12", "Ballari"],
  ["13", "Chitradurga"], ["14", "Davanagere"], ["15", "Shivamogga"], ["16", "Udupi"],
  ["17", "Chikkamagaluru"], ["18", "Tumakuru"], ["19", "Kolara"],
  ["20", "Bengaluru (Urban)"], ["21", "Bengaluru (Rural)"], ["22", "Mandya"],
  ["23", "Hassan"], ["24", "Dakshina Kannada"], ["25", "Kodagu"], ["26", "Mysuru"],
  ["27", "Chamarajanagara"], ["28", "Chikkaballapura"], ["29", "Bengaluru South"],
  ["30", "Yadgir"], ["31", "Vijayanagara"],
];
...
export async function GET(request: NextRequest) {
  ...
  if (!namesMatch(state, "Karnataka")) {
    return NextResponse.json({ error: `No hobli data available for state "${state}"` }, { status: 404 });
  }
  ...
  try {
    const [taluks, hoblies] = await Promise.all([
      readGeoJson(`${PREFIX}KARNATAKA_TALUKS.geojson`),
      readGeoJson(`${PREFIX}KARNATAKA_HOBLIS.geojson`),
    ]);
    ...
    if (!talukCode) {
      return NextResponse.json({ error: `Taluk "${taluk}" was not found in district "${district}"` }, { status: 404 });
    }

    const features = hoblies.features.filter(
    ...
    console.error("[taluk-hoblies] Failed to load updated hobli boundaries:", error);
