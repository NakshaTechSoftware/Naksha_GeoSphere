...
import { openMeteoCurrentProvider, openMeteoForecastProvider } from "@/lib/weather/weatherPipelines";
import {
  fieldLegendStops,
  fieldUnit,
...
// IMD warnings sit above every other overlay (NASA imagery, weather rasters,
// rain, boundaries) so they stay legible, but still below text labels -
// unlike weather imagery/boundaries, which anchor on the boundary line
// specifically, warnings just need "before the first label", i.e. on top of
// everything non-text.
function findImdWarningInsertionPoint(map: MapLibreMap): string | undefined {
  const layers = map.getStyle().layers ?? [];
  return layers.find((l: any) => l.type === "symbol" || l.id?.includes("label"))?.id;
}
...
function featureMatchesName(feature: MapGeoJSONFeature, names: string[]) {
  ...
  return candidates.some(
    (value) => typeof value === "string" && names.includes(value.trim().toLowerCase()),
  );
}
...
async function fitBoundsToGeoJSON(
  map: MapLibreMap,
  data: GeoJSON.FeatureCollection,
): Promise<void> {
...
export type PoliceType =
  ...
  | "traffic_police"
  | "railway_police"
  | "railway_police_outpost"
  | "police_outpost"
  | "police_check_post"
  | "police_forest_cell"
  | "district_armed_reserve"
  | "city_armed_reserve"
  | "city_crime_branch"
  | "coastal_security"
  | "cyber_crime"
...
export interface IndiaMapViewerProps {
  ...
  onDrillContextChange?: (
    context: { state: string; district: string; taluk: string } | null,
  ) => void;
  ...
  /** Optional caller-controlled point to pin/highlight on the map. */
  highlightedLocation?: {
    latitude: number;
    longitude: number;
    label?: string;
...
const ADMIN_BOUNDARY_LAYER_IDS = [...STATE_BOUNDARY_LAYER_IDS, ...BOUNDARY_LAYER_IDS];
...
function addDefaultBaseLayers(
  ...
  if (!map.getLayer(OSM_NOLABELS_LAYER_ID)) {
    map.addLayer(
      { id: OSM_NOLABELS_LAYER_ID, type: "raster", source: OSM_NOLABELS_SOURCE_ID },
      beforeId,
    );
  }
  ...
  if (!map.getLayer(OSM_LABELED_LAYER_ID)) {
    map.addLayer(
      { id: OSM_LABELED_LAYER_ID, type: "raster", source: OSM_LABELED_SOURCE_ID },
      beforeId,
    );
  }
...
function updateBaseLabelVisibility(
  ...
  if (map.getLayer(STATE_DISTRICTS_LABELS_LAYER_ID)) {
    const showDistrictLabels =
      boundariesVisible && scaleKm >= DISTRICT_LABEL_MIN_KM && scaleKm <= DISTRICT_LABEL_MAX_KM;
...
// --- Weather click panel ---------------------------------------------------
//
// Left-side "Weather" icon arms a single "click map -> details" mode. A left
// click then fetches the full current-conditions + AQI + forecast bundle for
// that exact coordinate into a fixed right-side panel (no per-metric
// selection). Drill-down clicks are paused while this mode is active.
...
// IMD district nowcast warnings (additive, independent overlay - see the
// dedicated effects below and findImdWarningInsertionPoint above).
const IMD_WARNING_SOURCE_ID = "imd-weather-warnings";
const IMD_WARNING_FILL_LAYER_ID = "imd-weather-warnings-fill";
const IMD_WARNING_OUTLINE_LAYER_ID = "imd-weather-warnings-outline";
...
const HIGHLIGHT_LOCATION_SOURCE_ID = "highlight-location-source";
const HIGHLIGHT_LOCATION_RING_LAYER_ID = "highlight-location-ring-layer";
const HIGHLIGHT_LOCATION_DOT_LAYER_ID = "highlight-location-dot-layer";
const HIGHLIGHT_LOCATION_LABEL_LAYER_ID = "highlight-location-label-layer";
...
function WeatherMetaTile({ label, value }: { label: string; value: string }) {
  return (
...
function inferWeatherLocationLabel(map: MapLibreMap, point: PointLike): string | null {
  const lookups: Array<{
    layerId: string;
    primaryKeys: string[];
    parentKeys?: string[][];
    ...
    {
      layerId: HOBLI_VILLAGES_FILL_LAYER_ID,
      primaryKeys: ["KGISVillageName", "village_name", "Village_name", "name"],
      parentKeys: [
        ["KGISHobliName", "hobli_name", "name"],
        ["Taluk_name", "taluk_name", "subdistrict", "subdist_nm"],
      ],
    },
    {
      layerId: TALUK_HOBLIES_FILL_LAYER_ID,
      primaryKeys: ["KGISHobliName", "hobli_name", "Hobli_name", "name"],
      parentKeys: [["Taluk_name", "taluk_name", "subdistrict", "subdist_nm"]],
    },
    {
      layerId: DISTRICT_TALUKS_FILL_LAYER_ID,
      primaryKeys: ["subdistrict", "subdist_nm", "Taluk_name", "taluk_name", "name"],
      parentKeys: [["district", "district_name", "dtname", "name"]],
    },
    {
      layerId: STATE_DISTRICTS_FILL_LAYER_ID,
      primaryKeys: ["district", "district_name", "dtname", "name"],
      parentKeys: [["state", "state_name", "stname", "STATE", "name"]],
    },
    {
      layerId: "states-fill-default",
      primaryKeys: ["state", "state_name", "stname", "STATE", "name"],
    },
  ];

  for (const lookup of lookups) {
    // Skip layers that aren't present in the current style (e.g. boundary
    // fill layers that haven't loaded yet) instead of hitting the
    // "layer does not exist" throw from queryRenderedFeatures.
    if (!map.getLayer(lookup.layerId)) continue;
    const feature = queryRenderedFeaturesSafe(map, point, {
      layers: [lookup.layerId],
    })[0];
    if (!feature) continue;
    const primary = firstNamedProperty(feature.properties ?? {}, lookup.primaryKeys);
    if (!primary) continue;
    const parents =
      lookup.parentKeys
        ?.map((keys) => firstNamedProperty(feature.properties ?? {}, keys))
        .filter((value): value is string => Boolean(value) && value !== primary) ?? [];
    const label = [primary, ...parents.slice(0, 1)];
    return label.join(", ");
  }
...
export const IndiaMapViewer = forwardRef<IndiaMapViewerHandle, IndiaMapViewerProps>(
  ...
  useEffect(() => {
    ...
    // weatherDetailsActiveRef mirrors the armed state into the map's click-handler
    // closures, which are only set up once on mount.
    const [showWeatherMenu, setShowWeatherMenu] = useState(false);
    // Single toggle that arms "click map -> weather details". Replaces the old
    // six per-metric checkboxes - a map click now always returns the full
    // current-conditions + AQI + forecast bundle, with no metric pre-selection.
    const [weatherDetailsActive, setWeatherDetailsActive] = useState(false);
    ...
    const pendingWindPointRef = useRef<{ x: number; y: number; lng: number; lat: number } | null>(
      null,
    );
  ...
  // simply follow their layers' visibility.
    ...
    const [weatherFieldStatus, setWeatherFieldStatus] = useState<
    ...
    const [himawariStatus, setHimawariStatus] = useState<
      "idle" | "loading" | "ready" | "unavailable"
    >("idle");
    ...
    const [goesWestStatus, setGoesWestStatus] = useState<
      "idle" | "loading" | "ready" | "unavailable"
    >("idle");
    ...
    const [goesEastStatus, setGoesEastStatus] = useState<
      "idle" | "loading" | "ready" | "unavailable"
    >("idle");
    ...
    const [imergStatus, setImergStatus] = useState<"idle" | "loading" | "ready" | "unavailable">(
      "idle",
    );
    const [lstDayNightSelection, setLstDayNightSelection] = useState<"auto" | ViirsLstDayNight>(
      "auto",
    );
    const [lstPlatform, setLstPlatform] = useState<"auto" | "SNPP" | "NOAA20" | "NOAA21">("auto");
    const [lstResolvedDay, setLstResolvedDay] = useState<ResolvedViirsLst | null | undefined>(
      undefined,
    );
    const [lstResolvedNight, setLstResolvedNight] = useState<ResolvedViirsLst | null | undefined>(
      undefined,
    );
    const [lstDate, setLstDate] = useState<string | null>(null);
    const [lstAvailableDates, setLstAvailableDates] = useState<string[]>([]);
    const [lstStatus, setLstStatus] = useState<"idle" | "loading" | "ready" | "unavailable">(
      "idle",
    );
    ...
    const [satelliteResolved, setSatelliteResolved] = useState<
      ResolvedGibsSatellite | null | undefined
    >(undefined);
    ...
    const [ndviStatus, setNdviStatus] = useState<"idle" | "loading" | "ready" | "unavailable">(
      "idle",
    );

    // NASA FIRMS active-fire detections (Fire mode).
    const [fireDetections, setFireDetections] = useState<FireDetection[]>([]);
    const [fireStatus, setFireStatus] = useState<"idle" | "loading" | "ready" | "unavailable">(
      "idle",
    );
    ...
    const [aqiStatus, setAqiStatus] = useState<"idle" | "loading" | "ready" | "unavailable">(
      "idle",
    );
    const [aqiMessage, setAqiMessage] = useState<string | null>(null);
    const lastFocusedHighlightedLocationRef = useRef<string | null>(null);

    // IMD district nowcast warnings - a fully independent, additive overlay
    // (default off). Fetched once on activation (not on every map move/zoom;
    // the backend itself caches for 10 min - see imd_warnings.py), never on
    // NASA/rain/wind/weather-mode changes. See findImdWarningInsertionPoint
    // for why this renders above boundaries but below labels.
    const [showImdWarnings, setShowImdWarnings] = useState(false);
    const showImdWarningsRef = useRef(false);
    const [imdWarnings, setImdWarnings] = useState<ImdWarningsResponse | null>(null);
    const [imdWarningsStatus, setImdWarningsStatus] = useState<
      "idle" | "loading" | "ready" | "unavailable"
    >("idle");
    const [selectedImdWarning, setSelectedImdWarning] = useState<{
    ...
    // its result is simply never applied once a newer selection exists.
    const weatherGenerationRef = useRef(0);

    // Fetches current weather, forecast, and AQI for a clicked point as three
    // independent pipelines (current+AQI share one Open-Meteo call today, but
    // are still tracked and rendered independently - see weatherPipelines.ts).
    // Shared by the map click handler and the panel's Retry button. Each
    // pipeline applies its own result the moment it resolves, so a slow
    // forecast fetch never blocks current weather (or AQI) from appearing,
    // and one failing never clears what the others already loaded.
    const loadWeatherAtPoint = useCallback(
      (lat: number, lng: number, locationLabel: string | null) => {
        ...
        setWeatherPanel({
          ...
          generation,
          location,
          current: { status: "loading", data: null },
          forecast: { status: "loading", data: null },
          aqi: { status: "loading", official: null, modeled: null, distanceKm: null },
        });
        ...
        openMeteoCurrentProvider.getCurrent(lat, lng).then(
          ...
          () => {
            ...
            setWeatherPanel((prev) =>
              prev.status !== "active" || prev.generation !== generation
                ? prev
                : {
                    ...prev,
                    current: { status: "error", data: null },
                    aqi: { status: "error", official: null, modeled: null, distanceKm: null },
                  },
            );
          },
        );

        openMeteoForecastProvider.getForecast(lat, lng).then(
          (forecast) => {
            if (weatherGenerationRef.current !== generation) return;
            setWeatherPanel((prev) =>
              prev.status !== "active" || prev.generation !== generation
                ? prev
                : { ...prev, forecast: { status: "success", data: forecast } },
            );
          },
          () => {
            if (weatherGenerationRef.current !== generation) return;
            setWeatherPanel((prev) =>
              prev.status !== "active" || prev.generation !== generation
                ? prev
                : { ...prev, forecast: { status: "error", data: null } },
            );
          },
        );
      },
    ...
    const handleWeatherRetry = useCallback(() => {
      ...
      loadWeatherAtPoint(wp.location.latitude, wp.location.longitude, wp.location.locationLabel);
    }, [weatherPanel, loadWeatherAtPoint]);

    // Cross-references the Weather Details panel's selected location against
    // active IMD warnings, purely additively - this is a separate derived
    // value passed as its own prop, never merged into weatherPanel.current/
    // forecast/aqi (see WeatherPanelState docs). Memoized: a full India-wide
    // point-in-polygon scan on every render (e.g. while the wind cursor is
    // sampling on mousemove) would be wasteful - only recomputes when the
    // selected location or the warnings data actually changes.
    const imdWarningAtSelectedLocation = useMemo<ImdWarningProperties | null>(() => {
      if (weatherPanel.status !== "active" || !imdWarnings) return null;
      const pt = turfPoint([weatherPanel.location.longitude, weatherPanel.location.latitude]);
      for (const feature of imdWarnings.features) {
        try {
          if (booleanPointInPolygon(pt, feature.geometry as any)) {
            return feature.properties;
          }
        } catch {
          // Malformed geometry for this one feature - skip it, never crash
          // the panel over it.
        }
      }
      return null;
    }, [weatherPanel, imdWarnings]);

    useEffect(() => {
      weatherDetailsActiveRef.current = weatherDetailsActive;
    }, [weatherDetailsActive]);
    useEffect(() => {
      showImdWarningsRef.current = showImdWarnings;
    }, [showImdWarnings]);
    // IMD warnings are an "inside weather" feature that overlaps the click-map
    // details workflow. When weather (click-details) mode is turned off, force
    // the IMD warnings overlay off and disable its toggle.
    useEffect(() => {
    ...
    // below (details armed or radar/wind toggled on both count, even with the dropdown
    ...
    }, [showWeatherMenu, weatherMode, weatherDetailsActive, isRadarEnabled, isWindEnabled]);
    useEffect(() => {
  ...
  const applyBoundaryLayerVisibility = (map: MapLibreMap) => {
    ...
    const villageNameKey = () => {
      ...
      return VILLAGE_NAME_KEYS.find((key) => typeof firstProps[key] === "string") ?? "name";
    };
    ...
    const loadedRoadsHighwaysRef = useRef<{
    ...
    const clearStateDistricts = (map: MapLibreMap) => {
      if (map.getLayer(STATE_DISTRICTS_FILL_LAYER_ID))
        map.removeLayer(STATE_DISTRICTS_FILL_LAYER_ID);
      if (map.getLayer(STATE_DISTRICTS_LINE_LAYER_ID))
        map.removeLayer(STATE_DISTRICTS_LINE_LAYER_ID);
      ...
      if (map.getSource(STATE_DISTRICTS_LABELS_SOURCE_ID))
        map.removeSource(STATE_DISTRICTS_LABELS_SOURCE_ID);
    ...
    const clearGbaCorporations = (map: MapLibreMap) => {
      if (map.getLayer(GBA_CORPORATIONS_FILL_LAYER_ID))
        map.removeLayer(GBA_CORPORATIONS_FILL_LAYER_ID);
      if (map.getLayer(GBA_CORPORATIONS_LINE_LAYER_ID))
        map.removeLayer(GBA_CORPORATIONS_LINE_LAYER_ID);
      ...
      if (map.getSource(GBA_CORPORATIONS_LABELS_SOURCE_ID))
        map.removeSource(GBA_CORPORATIONS_LABELS_SOURCE_ID);
    ...
    const clearKarnatakaStateBoundary = (map: MapLibreMap) => {
      if (map.getLayer(KARNATAKA_STATE_FILL_LAYER_ID))
        map.removeLayer(KARNATAKA_STATE_FILL_LAYER_ID);
      if (map.getLayer(KARNATAKA_STATE_LINE_LAYER_ID))
        map.removeLayer(KARNATAKA_STATE_LINE_LAYER_ID);
      ...
      if (map.getSource(KARNATAKA_STATE_LABELS_SOURCE_ID))
        map.removeSource(KARNATAKA_STATE_LABELS_SOURCE_ID);
      ...
      try {
        ...
        const karnatakaData: GeoJSON.FeatureCollection = {
          ...
          features: statesData.features.filter(
            (f) =>
              (f.properties?.st_nm as string | undefined)?.trim().toLowerCase() === "karnataka",
          ),
        };

        map.addSource(KARNATAKA_STATE_SOURCE_ID, {
          type: "geojson",
          data: karnatakaData,
          generateId: true,
        });
    ...
    const clearGbaBoundary = (map: MapLibreMap) => {
      ...
      if (map.getSource(GBA_BOUNDARY_LABELS_SOURCE_ID))
        map.removeSource(GBA_BOUNDARY_LABELS_SOURCE_ID);
    ...
    const loadGbaCorporations = async (map: MapLibreMap) => {
      ...
      try {
        ...
        map.addSource(GBA_CORPORATIONS_SOURCE_ID, {
          type: "geojson",
          data: dataWithIds,
          generateId: false,
        });
    ...
    const loadGbaZones = async (map: MapLibreMap, corporation: string) => {
      ...
      try {
        const response = await fetch(
          `/api/datasets/gba-zones?corporation=${encodeURIComponent(corporation)}`,
        );
        ...
        map.addSource(GBA_ZONES_SOURCE_ID, {
          type: "geojson",
          data: dataWithIds,
          generateId: false,
        });
    ...
    const loadGbaWards = async (map: MapLibreMap, corporation: string, zone: string) => {
      ...
      try {
        ...
        map.addSource(GBA_WARDS_SOURCE_ID, {
          type: "geojson",
          data: dataWithIds,
          generateId: false,
        });
        ...
        loadedGbaWardsZoneRef.current = {
          corporation: corporation.trim().toLowerCase(),
    ...
    const clearRoadsHighways = (map: MapLibreMap) => {
      ...
      if (map.getLayer(ROADS_LOCAL_ROADS_LINE_LAYER_ID))
        map.removeLayer(ROADS_LOCAL_ROADS_LINE_LAYER_ID);
      ...
      if (map.getSource(ROADS_STATEWIDE_HIGHWAYS_SOURCE_ID))
        map.removeSource(ROADS_STATEWIDE_HIGHWAYS_SOURCE_ID);
    ...
    const clearRoadsVillages = (map: MapLibreMap) => {
      ...
      if (map.getSource(ROADS_VILLAGES_LABELS_SOURCE_ID))
        map.removeSource(ROADS_VILLAGES_LABELS_SOURCE_ID);
    ...
    const clearRoadsHoblies = (map: MapLibreMap) => {
      ...
      if (map.getSource(ROADS_HOBLIES_LABELS_SOURCE_ID))
        map.removeSource(ROADS_HOBLIES_LABELS_SOURCE_ID);
    ...
    const clearRoadsTaluks = (map: MapLibreMap) => {
      ...
      if (map.getSource(ROADS_TALUKS_LABELS_SOURCE_ID))
        map.removeSource(ROADS_TALUKS_LABELS_SOURCE_ID);
    ...
    const clearRoadsDistricts = (map: MapLibreMap) => {
      ...
      if (map.getLayer(ROADS_DISTRICTS_FILL_LAYER_ID))
        map.removeLayer(ROADS_DISTRICTS_FILL_LAYER_ID);
      if (map.getLayer(ROADS_DISTRICTS_LINE_LAYER_ID))
        map.removeLayer(ROADS_DISTRICTS_LINE_LAYER_ID);
      ...
      if (map.getSource(ROADS_DISTRICTS_LABELS_SOURCE_ID))
        map.removeSource(ROADS_DISTRICTS_LABELS_SOURCE_ID);
    ...
    const loadRoadsHighways = async (
      map: MapLibreMap,
      level: "district" | "taluk",
      district: string,
      taluk?: string,
    ) => {
      ...
      try {
        ...
        const localRoadsData: GeoJSON.FeatureCollection = {
          type: "FeatureCollection",
          features: localRoadsFeatures,
        };
    ...
    const applyRoadsBoundaryFilter = (
      map: MapLibreMap,
    ...
    const clearStateAssembly = (map: MapLibreMap) => {
      ...
      if (map.getSource(STATE_ASSEMBLY_LABELS_SOURCE_ID))
        map.removeSource(STATE_ASSEMBLY_LABELS_SOURCE_ID);
      loadedAssemblyStateRef.current = null;
      selectedAssemblyIdRef.current = null;
    };

    const clearStateParliament = (map: MapLibreMap) => {
      if (map.getLayer(STATE_PARLIAMENT_FILL_LAYER_ID))
        map.removeLayer(STATE_PARLIAMENT_FILL_LAYER_ID);
      if (map.getLayer(STATE_PARLIAMENT_LINE_LAYER_ID))
        map.removeLayer(STATE_PARLIAMENT_LINE_LAYER_ID);
      ...
      if (map.getSource(STATE_PARLIAMENT_LABELS_SOURCE_ID))
        map.removeSource(STATE_PARLIAMENT_LABELS_SOURCE_ID);
    ...
    const clearStatePolice = (map: MapLibreMap) => {
      ...
      if (map.getLayer(STATE_POLICE_POINT_HALO_LAYER_ID))
        map.removeLayer(STATE_POLICE_POINT_HALO_LAYER_ID);
      if (map.getLayer(STATE_POLICE_POINT_LABEL_LAYER_ID))
        map.removeLayer(STATE_POLICE_POINT_LABEL_LAYER_ID);
    ...
    const clearGpBoundaries = (map: MapLibreMap) => {
      ...
      if (map.getSource(GP_BOUNDARIES_LABELS_SOURCE_ID))
        map.removeSource(GP_BOUNDARIES_LABELS_SOURCE_ID);
    ...
    const clearCivicPincodes = (map: MapLibreMap) => {
      ...
      if (map.getSource(CIVIC_PINCODES_LABELS_SOURCE_ID))
        map.removeSource(CIVIC_PINCODES_LABELS_SOURCE_ID);
      loadedCivicPincodesDistrictRef.current = null;
    };

    const clearCivicDistricts = (map: MapLibreMap) => {
      if (map.getLayer(CIVIC_DISTRICTS_FILL_LAYER_ID))
        map.removeLayer(CIVIC_DISTRICTS_FILL_LAYER_ID);
      if (map.getLayer(CIVIC_DISTRICTS_LINE_LAYER_ID))
        map.removeLayer(CIVIC_DISTRICTS_LINE_LAYER_ID);
      ...
      if (map.getSource(CIVIC_DISTRICTS_LABELS_SOURCE_ID))
        map.removeSource(CIVIC_DISTRICTS_LABELS_SOURCE_ID);
    ...
    const clearGpDistricts = (map: MapLibreMap) => {
      ...
      if (map.getSource(GP_DISTRICTS_LABELS_SOURCE_ID))
        map.removeSource(GP_DISTRICTS_LABELS_SOURCE_ID);
    ...
    const loadStateAssembly = async (map: MapLibreMap, stateName: string) => {
      ...
      try {
        const response = await fetch(
          `/api/datasets/state-assembly?state=${encodeURIComponent(stateName)}`,
        );
        if (!response.ok) {
        ...
        attachLabelHoverGrow(
          map,
          STATE_ASSEMBLY_LABELS_LAYER_ID,
          `${STATE_ASSEMBLY_LABELS_LAYER_ID}-hover`,
        );
    ...
    const loadStateParliament = async (map: MapLibreMap, stateName: string) => {
      ...
      try {
        const response = await fetch(
          `/api/datasets/state-parliament?state=${encodeURIComponent(stateName)}`,
        );
        if (!response.ok) {
        ...
        attachLabelHoverGrow(
          map,
          STATE_PARLIAMENT_LABELS_LAYER_ID,
          `${STATE_PARLIAMENT_LABELS_LAYER_ID}-hover`,
        );
        ...
        console.error(
          `Failed to load parliamentary constituency boundaries for "${stateName}":`,
          error,
        );
    ...
    const loadStatePolice = async (
      map: MapLibreMap,
      stateName: string,
      policeType = policeTypeRef.current,
      district = policeDistrictRef.current,
    ) => {
    ...
    const loadStateGramPanchayatDistricts = async (map: MapLibreMap, stateName: string) => {
      ...
      try {
        ...
        console.error(
          `Failed to load gram panchayat district boundaries for "${stateName}":`,
          error,
        );
    ...
    const loadStateCivicDistricts = async (map: MapLibreMap, stateName: string) => {
      ...
      try {
        ...
        console.error(
          `Failed to load civic amenities district boundaries for "${stateName}":`,
          error,
        );
    ...
    const loadDistrictCivicPincodes = async (
      ...
      try {
        ...
        console.error(
          `Failed to load civic amenities pincode boundaries for "${districtName}":`,
          error,
        );
    ...
    const loadDistrictGramPanchayatTaluks = async (
      ...
      try {
        ...
        console.error(
          `Failed to load gram panchayat taluk boundaries for "${districtName}":`,
          error,
        );
    ...
    const clearTalukHoblies = (map: MapLibreMap) => {
      ...
      if (map.getSource(TALUK_HOBLIES_LABELS_SOURCE_ID))
        map.removeSource(TALUK_HOBLIES_LABELS_SOURCE_ID);
    ...
    const clearVillageCadastrals = (map: MapLibreMap) => {
      if (map.getLayer(VILLAGE_CADASTRALS_FILL_LAYER_ID))
        map.removeLayer(VILLAGE_CADASTRALS_FILL_LAYER_ID);
      if (map.getLayer(VILLAGE_CADASTRALS_LINE_LAYER_ID))
        map.removeLayer(VILLAGE_CADASTRALS_LINE_LAYER_ID);
      if (map.getLayer(VILLAGE_CADASTRALS_LABELS_LAYER_ID))
        map.removeLayer(VILLAGE_CADASTRALS_LABELS_LAYER_ID);
      if (map.getSource(VILLAGE_CADASTRALS_SOURCE_ID))
        map.removeSource(VILLAGE_CADASTRALS_SOURCE_ID);
    ...
    const clearHobliVillages = (map: MapLibreMap) => {
      ...
      if (map.getSource(HOBLI_VILLAGES_LABELS_SOURCE_ID))
        map.removeSource(HOBLI_VILLAGES_LABELS_SOURCE_ID);
    ...
    const clearDistrictTaluks = (map: MapLibreMap) => {
      ...
      if (map.getLayer(DISTRICT_TALUKS_FILL_LAYER_ID))
        map.removeLayer(DISTRICT_TALUKS_FILL_LAYER_ID);
      if (map.getLayer(DISTRICT_TALUKS_LINE_LAYER_ID))
        map.removeLayer(DISTRICT_TALUKS_LINE_LAYER_ID);
      ...
      if (map.getSource(DISTRICT_TALUKS_LABELS_SOURCE_ID))
        map.removeSource(DISTRICT_TALUKS_LABELS_SOURCE_ID);
        ...
        if (data) {
          ...
          if (taluksData.features) {
            ...
            taluksData.features.forEach((f: any, idx: number) => {
              console.log(
                `  ${idx + 1}. KGISTalukName="${f.properties?.KGISTalukName}", KGISTalukCode="${f.properties?.KGISTalukCode}", subdist_nm="${f.properties?.subdist_nm}"`,
              );
            });
          }
        }
    ...
    const loadTalukHoblies = async (
      ...
      console.log(
        `Loading hoblies for taluk: ${talukName}, district: ${districtName}, state: ${stateName}`,
      );
    ...
    const loadHobliVillages = async (
      ...
      console.log(
        `Loading villages for hobli: ${hobliName}, taluk: ${talukName}, district: ${districtName}, state: ${stateName}`,
      );
    ...
    const loadVillageCadastrals = async (
      ...
      console.log(
        `Loading cadastrals for village: ${villageName}, hobli: ${hobliName}, taluk: ${talukName}, district: ${districtName}, state: ${stateName}`,
      );
    ...
    const captureDrillSnapshot = (map: MapLibreMap): DrillSnapshot => {
      ...
      ): DrillLevel | null => (loaded && data ? { parent, selectedId, selectedName, data } : null);
    ...
    const selectDistrictByName = async (
      ...
      let index = data.features.findIndex(
        (f) =>
          normalizeNameForMatch((f.properties?.dtname as string | undefined) ?? "") === normalized,
      );
      if (index === -1) {
      ...
      setFeatureStateSafe(
        map,
        { source: STATE_DISTRICTS_SOURCE_ID, id: index },
        { selected: true },
      );
    ...
    const selectTalukByName = async (
      ...
      const index = data.features.findIndex(
        (f) =>
          normalizeNameForMatch((f.properties?.KGISTalukName as string | undefined) ?? "") ===
          normalized,
      );
      if (index === -1) return false;
      const feature = data.features[index];
      ...
      setFeatureStateSafe(
        map,
        { source: DISTRICT_TALUKS_SOURCE_ID, id: index },
        { selected: true },
      );
      ...
      const actualTalukName =
        (feature.properties?.KGISTalukName as string | undefined) ?? talukName;
    ...
    const selectHobliByName = async (
      ...
      let index = data.features.findIndex(
        (f) =>
          normalizeNameForMatch((f.properties?.KGISHobliName as string | undefined) ?? "") ===
          normalized,
      );
      if (index === -1) {
        const prefixMatches = data.features
      ...
      const actualHobliName =
        (feature.properties?.KGISHobliName as string | undefined) ?? hobliName;
    ...
    const selectVillageByName = async (
      ...
      if (!(await selectHobliByName(map, stateName, districtName, talukName, hobliName)))
        return false;
      ...
      let index = data.features.findIndex(
        (f) =>
          normalizeNameForMatch((f.properties?.[key] as string | undefined) ?? "") === normalized,
      );
      if (index === -1) {
        const prefixMatches = data.features
          .map((f, i) => ({
          ...
          map.addLayer(
            {
              ...
              paint: {
                "raster-opacity":
                  weatherMode === "pressure"
                    ? DISPLAY_OPACITY.pressure
                    : weatherMode === "rain"
                      ? DISPLAY_OPACITY.observedRain
                      : DISPLAY_OPACITY.observedCloud,
                "raster-fade-duration": 0,
                "raster-resampling": "linear",
              },
            },
    ...
    useEffect(() => {
      ...
      if (showPressureExtrema && pressureExtrema.length > 0) {
        ...
        if (!map.getSource(PRESSURE_EXTREMA_SOURCE_ID)) {
          map.addSource(PRESSURE_EXTREMA_SOURCE_ID, {
            type: "geojson",
            data: extremaGeojson as any,
          });
          map.addLayer({
          ...
          (map.getSource(PRESSURE_EXTREMA_SOURCE_ID) as GeoJSONSource).setData(
            extremaGeojson as any,
          );
    ...
    useEffect(() => {
      ...
      resolveGibsSatellite().then(async (resolved) => {
        ...
        const checks = await Promise.all(
          candidates.map((d) =>
            probeGibsDate(resolved.product.layer, d).then((ok) => (ok ? d : null)),
          ),
        );
        if (cancelled) return;
    ...
    useEffect(() => {
      ...
      if (!map.getSource(GIBS_SATELLITE_SOURCE_ID)) {
        ...
        (
          map.getSource(GIBS_SATELLITE_SOURCE_ID) as { setTiles?: (t: string[]) => void }
        ).setTiles?.([tileUrl]);
    ...
    useEffect(() => {
      ...
      if (himawariFrames.length > 0 && Date.now() - himawariResolvedAtRef.current < staleAfterMs)
        return;
      let cancelled = false;
  ...
  useEffect(() => {
    ...
    useEffect(() => {
      ...
      const applyCloudFill = () => {
        try {
          ...
          const satelliteLayerOrder = [HIMAWARI_LAYER_ID, GOES_WEST_LAYER_ID, GOES_EAST_LAYER_ID];
          ...
          if (!insertBeforeId) {
            insertBeforeId = map
              .getStyle()
              .layers?.find((l: any) => l.type === "symbol" || l.id?.includes("label"))?.id;
          }
    ...
    useEffect(() => {
      ...
      if (!map.getSource(IMERG_SOURCE_ID)) {
        ...
        (map.getSource(IMERG_SOURCE_ID) as { setTiles?: (t: string[]) => void }).setTiles?.([
          tileUrl,
        ]);
    ...
    useEffect(() => {
      ...
      if (!map.getSource(NDVI_SOURCE_ID)) {
        ...
        (map.getSource(NDVI_SOURCE_ID) as { setTiles?: (t: string[]) => void }).setTiles?.([
          tileUrl,
        ]);
    ...
    useEffect(() => {
      ...
      const removeAll = () => {
        [
          ...
          FIRE_CLUSTER_COUNT_LAYER_ID,
          FIRE_CLUSTER_LAYER_ID,
        ].forEach((id) => {
          if (map.getLayer(id)) map.removeLayer(id);
        });
    ...
    // ── IMD district nowcast warnings ───────────────────────────────────────────
    // Independent, additive, off-by-default overlay - fetched once on
    // activation (never on map move/zoom; India-wide data, and the backend
    // itself caches for 10 min - see imd_warnings.py) and left untouched by
    // every other weather/NASA/rain/wind mode change.
    useEffect(() => {
      if (!showImdWarnings) {
        setImdWarnings(null);
        setImdWarningsStatus("idle");
        setSelectedImdWarning(null);
        return;
      }
      let cancelled = false;
      setImdWarningsStatus("loading");
      fetchImdWarnings()
        .then((data) => {
          if (cancelled) return;
          setImdWarnings(data);
          setImdWarningsStatus("ready");
        })
        .catch(() => {
          if (cancelled) return;
          // Quietly unavailable - NASA/rain/wind/current weather/forecast/AQI
          // must all keep working regardless (see non-regression rule).
          setImdWarnings(null);
          setImdWarningsStatus("unavailable");
        });
      return () => {
        cancelled = true;
      };
    ...
    // Renders the fetched warnings as a fill + outline layer, styled by the
    // source's own severity (never by re-deriving color from anything else).
    useEffect(() => {
      const map = mapRef.current;
      if (!map) return;

      const removeAll = () => {
        if (map.getLayer(IMD_WARNING_OUTLINE_LAYER_ID)) map.removeLayer(IMD_WARNING_OUTLINE_LAYER_ID);
        if (map.getLayer(IMD_WARNING_FILL_LAYER_ID)) map.removeLayer(IMD_WARNING_FILL_LAYER_ID);
        if (map.getSource(IMD_WARNING_SOURCE_ID)) map.removeSource(IMD_WARNING_SOURCE_ID);
      };

      if (!showImdWarnings || !imdWarnings || imdWarnings.features.length === 0) {
        removeAll();
        return;
      }

      const applyLayer = () => {
        const existingSource = map.getSource(IMD_WARNING_SOURCE_ID) as GeoJSONSource | undefined;
        if (existingSource) {
          existingSource.setData(imdWarnings as unknown as GeoJSON.FeatureCollection);
          return;
        }
        map.addSource(IMD_WARNING_SOURCE_ID, {
          type: "geojson",
          data: imdWarnings as unknown as GeoJSON.FeatureCollection,
        });
        const beforeId = findImdWarningInsertionPoint(map);
        map.addLayer(
          {
            id: IMD_WARNING_FILL_LAYER_ID,
            type: "fill",
            source: IMD_WARNING_SOURCE_ID,
            paint: {
              // GREEN stays essentially invisible (routine/no-warning districts
              // make up most of the dataset) - only an active watch/alert/
              // warning should visually stand out. Never fully opaque.
              "fill-color": [
                "match",
                ["get", "severity"],
                "RED",
                ...
                "GREEN",
                "#22c55e",
                "#94a3b8", // unrecognized/null severity - neutral, not a guess at meaning
              ],
              "fill-opacity": [
                "match",
                ["get", "severity"],
        ...
        map.addLayer(
          {
            id: IMD_WARNING_OUTLINE_LAYER_ID,
            type: "line",
            source: IMD_WARNING_SOURCE_ID,
            paint: {
              "line-color": [
      ...
      if (!map.isStyleLoaded()) {
        ...
        return () => map.off("load", applyLayer);
      }
      applyLayer();
      return () => {
        // Only tear down on unmount/toggle-off (handled by the removeAll()
        // branch above) - a data refresh reuses setData(), never a full
        // remove+re-add, so the layer never flickers/disappears mid-update.
      };
    }, [showImdWarnings, imdWarnings]);

    // Click-to-inspect a warning polygon - independent of (and never
    // interferes with) the weather click-to-inspect popup or boundary
    // drill-down click handlers (registered separately, elsewhere).
    useEffect(() => {
      const map = mapRef.current;
      if (!map || !showImdWarnings) return;

      const handleClick = (e: MapLayerMouseEvent) => {
        const feature = e.features?.[0];
        if (!feature) return;
        setSelectedImdWarning({
          properties: feature.properties as unknown as ImdWarningProperties,
          screenX: e.point.x,
          screenY: e.point.y,
        });
      };
      ...
      map.on("click", IMD_WARNING_FILL_LAYER_ID, handleClick);
      map.on("mouseenter", IMD_WARNING_FILL_LAYER_ID, handleEnter);
      map.on("mouseleave", IMD_WARNING_FILL_LAYER_ID, handleLeave);
      return () => {
        map.off("click", IMD_WARNING_FILL_LAYER_ID, handleClick);
        map.off("mouseenter", IMD_WARNING_FILL_LAYER_ID, handleEnter);
        map.off("mouseleave", IMD_WARNING_FILL_LAYER_ID, handleLeave);
      };
    }, [showImdWarnings]);
      ...
      const doResolve = async () => {
        ...
        const checks = await Promise.all(
          candidateDates.map((d) =>
            probeViirsLstDate(resolved.layer, d).then((ok) => (ok ? d : null)),
          ),
        );
        if (cancelled) return;
        const valid = checks.filter((d): d is string => d !== null);
    ...
    useEffect(() => {
      ...
      if (!map.getSource(VIIRS_LST_SOURCE_ID)) {
        ...
        (map.getSource(VIIRS_LST_SOURCE_ID) as { setTiles?: (t: string[]) => void }).setTiles?.([
          tileUrl,
        ]);
    ...
    useEffect(() => {
      ...
      const cleanupExisting = () => {
        ...
        [
          AQI_STATIONS_POINTS_LAYER_ID,
          AQI_STATIONS_CLUSTER_LAYER_ID,
          AQI_STATIONS_CLUSTER_COUNT_LAYER_ID,
        ].forEach((id) => {
          if (map.getLayer(id)) map.removeLayer(id);
        });
    ...
    useEffect(() => {
      ...
      const applyHighlightedLocation = () => {
        if (!highlightedLocation) {
          if (map.getLayer(HIGHLIGHT_LOCATION_LABEL_LAYER_ID)) {
            map.setLayoutProperty(HIGHLIGHT_LOCATION_LABEL_LAYER_ID, "visibility", "none");
          }
          if (map.getLayer(HIGHLIGHT_LOCATION_DOT_LAYER_ID)) {
            map.setLayoutProperty(HIGHLIGHT_LOCATION_DOT_LAYER_ID, "visibility", "none");
          }
          if (map.getLayer(HIGHLIGHT_LOCATION_RING_LAYER_ID)) {
            map.setLayoutProperty(HIGHLIGHT_LOCATION_RING_LAYER_ID, "visibility", "none");
          }
          lastFocusedHighlightedLocationRef.current = null;
          return;
        }
          ...
          features: [
            {
              type: "Feature",
              properties: {
                label: highlightedLocation.label ?? "Saved location",
              },
              geometry: {
                type: "Point",
                coordinates: [highlightedLocation.longitude, highlightedLocation.latitude],
              },
            },
          ],
        };

        if (!map.getSource(HIGHLIGHT_LOCATION_SOURCE_ID)) {
          map.addSource(HIGHLIGHT_LOCATION_SOURCE_ID, {
            type: "geojson",
            data: featureCollection,
          });

          const insertionPoint = findWeatherImageryInsertionPoint(map);
          map.addLayer(
            {
              id: HIGHLIGHT_LOCATION_RING_LAYER_ID,
              type: "circle",
              source: HIGHLIGHT_LOCATION_SOURCE_ID,
              paint: {
                "circle-radius": 14,
                "circle-color": "#2563eb",
                "circle-opacity": 0.14,
          ...
          map.addLayer(
            {
              id: HIGHLIGHT_LOCATION_DOT_LAYER_ID,
              type: "circle",
              source: HIGHLIGHT_LOCATION_SOURCE_ID,
              paint: {
                "circle-radius": 6,
          ...
          map.addLayer(
            {
              id: HIGHLIGHT_LOCATION_LABEL_LAYER_ID,
              type: "symbol",
              source: HIGHLIGHT_LOCATION_SOURCE_ID,
              layout: {
                "text-field": ["get", "label"],
          ...
          (map.getSource(HIGHLIGHT_LOCATION_SOURCE_ID) as GeoJSONSource).setData(featureCollection);
          map.setLayoutProperty(HIGHLIGHT_LOCATION_RING_LAYER_ID, "visibility", "visible");
          map.setLayoutProperty(HIGHLIGHT_LOCATION_DOT_LAYER_ID, "visibility", "visible");
          map.setLayoutProperty(HIGHLIGHT_LOCATION_LABEL_LAYER_ID, "visibility", "visible");
        }

        const focusKey = `${highlightedLocation.latitude}:${highlightedLocation.longitude}`;
        if (
          highlightedLocation.focusOnShow &&
          lastFocusedHighlightedLocationRef.current !== focusKey
        ) {
          lastFocusedHighlightedLocationRef.current = focusKey;
          map.easeTo({
      ...
      if (!map.isStyleLoaded()) {
        map.once("load", applyHighlightedLocation);
        return () => {
          map.off("load", applyHighlightedLocation);
        };
      }

      applyHighlightedLocation();
    }, [highlightedLocation]);
    ...
    useEffect(() => {
      ...
      async function initMap() {
        ...
        try {
          ...
          // Find My Way: place-click search (reverse geocode + boundary drill-down).
        ...
        map.on("moveend", () =>
          ...
          const reportAttributeInfo = (e: MapMouseEvent): boolean => {
            ...
            const features =
              layers.length > 0 ? queryRenderedFeaturesSafe(map, e.point, { layers }) : [];
            const feature = features[0];
            ...
            const title =
              nameKeys.map((k) => props[k]).find((v) => typeof v === "string" && v.trim()) ??
              typeLabel;
            ...
            // Source File path.
            const isGpBoundary =
              layerId === GP_BOUNDARIES_FILL_LAYER_ID || layerId === GP_BOUNDARIES_LINE_LAYER_ID;
            ...
            const stateName = typeof props.st_nm === "string" ? props.st_nm.trim() : "";
            ...
            const hierarchy: AttributeInfo["hierarchy"] = adminLevel
              ? {
                  level: adminLevel,
                  state:
                    adminLevel === "state" ? title : (selectedStateNameRef.current ?? undefined),
                  district:
                    adminLevel === "district"
                      ? title
          ...
          // Weather widget: once "click map details" is armed (weatherDetailsActive),
          // clicking anywhere on the map (that isn't a drill-down layer - those are
          // blocked by the isWeatherControlActiveRef guard added to each drill-down
          // click handler above) fetches current conditions + AQI + forecast for that
          // exact point and shows them in the fixed right-side panel. A monotonically
          // increasing request id discards any response that arrives after a newer
          // click superseded it.
          map.on("click", (e: MapMouseEvent) => {
            if (
              !weatherDetailsActiveRef.current ||
              drawingToolRef.current ||
              showImdWarningsRef.current
            )
              return;

            const { lng, lat } = e.lngLat;
            const locationLabel = inferWeatherLocationLabel(map, e.point);
            loadWeatherAtPoint(lat, lng, locationLabel);
          });
                ...
                try {
                  ...
                  map.on("click", "states-fill-default", (e) => {
                    ...
                    if (boundaryLoadedForState) {
                      console.log(
                        "Boundaries are loaded for this state - ignoring state click (click was on boundary layer)",
                      );
                      return;
                    }
                    ...
                    if (
                      ...
                      if (stateName) {
                        if (isAssemblyMode) {
                          console.log(
                            `Auto-loading assembly constituencies for state: ${stateName}`,
                          );
                          void loadStateAssembly(map, stateName);
                        } else if (isParliamentMode) {
                          console.log(
                            `Auto-loading parliamentary constituencies for state: ${stateName}`,
                          );
                          void loadStateParliament(map, stateName);
                        } else if (isPoliceMode) {
                          console.log(
                            `Auto-loading police station boundaries for state: ${stateName}`,
                          );
                          void loadStatePolice(map, stateName);
                        } else if (isGramPanchayatMode) {
                          console.log(
                            `Auto-loading gram panchayat districts for state: ${stateName}`,
                          );
                          void loadStateGramPanchayatDistricts(map, stateName);
                        } else if (isCivicAmenitiesMode) {
                          console.log(
                            `Auto-loading civic amenities districts for state: ${stateName}`,
                          );
                  ...
                  map.on("click", GP_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, { layers: [GP_TALUKS_FILL_LAYER_ID] })
                        .length > 0
                    ) {
                      return;
                    }
                  ...
                  map.on("click", CIVIC_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [CIVIC_PINCODES_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (
                      ...
                      pincodesAlreadyLoaded
                    ) {
                      setFeatureStateSafe(
                        map,
                        {
                          source: CIVIC_DISTRICTS_SOURCE_ID,
                          id: selectedCivicDistrictIdRef.current,
                        },
                        { selected: false },
                      );
                      selectedCivicDistrictIdRef.current = null;
                  ...
                  map.on("click", GP_TALUKS_FILL_LAYER_ID, (e) => {
                    ...
                    if (talukName && districtName && stateName) {
                      console.log(
                        `Loading GP boundaries for taluk ${talukName}, ${districtName}, ${stateName}`,
                      );
                      void loadTalukGramPanchayatBoundaries(
                        map,
                        talukName,
                        districtName,
                        stateName,
                  ...
                  map.on("mousemove", KARNATAKA_STATE_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: KARNATAKA_STATE_SOURCE_ID, id: hoveredKarnatakaStateId },
                        { hover: false },
                      );
                    }
                    hoveredKarnatakaStateId = feature.id;
                  ...
                  map.on("click", KARNATAKA_STATE_FILL_LAYER_ID, (e) => {
                    ...
                    if (mode === "gba") {
                      ...
                      if (
                        ...
                        queryRenderedFeaturesSafe(map, e.point, {
                          layers: [GBA_BOUNDARY_FILL_LAYER_ID],
                        }).length > 0
                      ) {
                        return;
                      }
                      ...
                      if (
                        ...
                        queryRenderedFeaturesSafe(map, e.point, {
                          layers: [ROADS_DISTRICTS_FILL_LAYER_ID],
                        }).length > 0
                      ) {
                        return;
                      }
                  ...
                  map.on("click", GBA_BOUNDARY_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [GBA_CORPORATIONS_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                  ...
                  map.on("mousemove", GBA_CORPORATIONS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: GBA_CORPORATIONS_SOURCE_ID, id: hoveredGbaCorporationId },
                        { hover: false },
                      );
                    }
                    hoveredGbaCorporationId = feature.id;
                  ...
                  map.on("click", GBA_CORPORATIONS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, { layers: [GBA_ZONES_FILL_LAYER_ID] })
                        .length > 0
                    ) {
                      return;
                    }
                    ...
                    if (
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: GBA_CORPORATIONS_SOURCE_ID, id: feature.id },
                        { selected: false },
                      );
                      selectedGbaCorporationIdRef.current = null;
                      clearGbaZones(map);
                    ...
                    if (selectedGbaCorporationIdRef.current !== null) {
                      setFeatureStateSafe(
                        ...
                        {
                          source: GBA_CORPORATIONS_SOURCE_ID,
                          id: selectedGbaCorporationIdRef.current,
                        },
                        { selected: false },
                      );
                    }
                  ...
                  map.on("mousemove", GBA_ZONES_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredGbaZoneId !== null && hoveredGbaZoneId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: GBA_ZONES_SOURCE_ID, id: hoveredGbaZoneId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", GBA_ZONES_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, { layers: [GBA_WARDS_FILL_LAYER_ID] })
                        .length > 0
                    ) {
                      return;
                    }
                    ...
                    if (!feature || feature.id === undefined) return;
                    const zoneName = (
                      (feature.properties?.zone_name ?? feature.properties?.Name) as
                        | string
                        | undefined
                    )?.trim();
                    ...
                    if (
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: GBA_ZONES_SOURCE_ID, id: feature.id },
                        { selected: false },
                      );
                      selectedGbaZoneIdRef.current = null;
                      clearGbaWards(map);
                    ...
                    if (selectedGbaZoneIdRef.current !== null) {
                      setFeatureStateSafe(
                        map,
                        { source: GBA_ZONES_SOURCE_ID, id: selectedGbaZoneIdRef.current },
                        { selected: false },
                      );
                    }
                    selectedGbaZoneIdRef.current = feature.id;
                  ...
                  map.on("mousemove", GBA_WARDS_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredGbaWardId !== null && hoveredGbaWardId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: GBA_WARDS_SOURCE_ID, id: hoveredGbaWardId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", GBA_WARDS_FILL_LAYER_ID, (e) => {
                    ...
                    if (selectedGbaWardIdRef.current !== null) {
                      setFeatureStateSafe(
                        map,
                        { source: GBA_WARDS_SOURCE_ID, id: selectedGbaWardIdRef.current },
                        { selected: false },
                      );
                    }
                    ...
                    if (!isSame) {
                      setFeatureStateSafe(
                        map,
                        { source: GBA_WARDS_SOURCE_ID, id: feature.id },
                        { selected: true },
                      );
                    }
                  });
                  ...
                  map.on("mousemove", ROADS_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredRoadsDistrictId !== null && hoveredRoadsDistrictId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_DISTRICTS_SOURCE_ID, id: hoveredRoadsDistrictId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", ROADS_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [ROADS_TALUKS_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_DISTRICTS_SOURCE_ID, id: feature.id },
                        { selected: false },
                      );
                      selectedRoadsDistrictIdRef.current = null;
                      selectedRoadsDistrictNameRef.current = null;
                    ...
                    if (selectedRoadsDistrictIdRef.current !== null) {
                      setFeatureStateSafe(
                        ...
                        {
                          source: ROADS_DISTRICTS_SOURCE_ID,
                          id: selectedRoadsDistrictIdRef.current,
                        },
                        { selected: false },
                      );
                    }
                  ...
                  map.on("dblclick", ROADS_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    const feature = e.features?.[0];
                    const districtName =
                      (feature?.properties?.dtname as string | undefined)?.trim() ??
                      selectedRoadsDistrictNameRef.current;
                    if (!districtName) return;
                    roadsSelectionGenerationRef.current++;
                    map.getCanvas().style.cursor = "wait";
                  ...
                  map.on("mousemove", ROADS_TALUKS_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredRoadsTalukId !== null && hoveredRoadsTalukId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_TALUKS_SOURCE_ID, id: hoveredRoadsTalukId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", ROADS_TALUKS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [ROADS_HOBLIES_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (selectedRoadsTalukIdRef.current === feature.id) {
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_TALUKS_SOURCE_ID, id: feature.id },
                        { selected: false },
                      );
                      selectedRoadsTalukIdRef.current = null;
                      selectedRoadsTalukNameRef.current = null;
                    ...
                    if (selectedRoadsTalukIdRef.current !== null) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_TALUKS_SOURCE_ID, id: selectedRoadsTalukIdRef.current },
                        { selected: false },
                      );
                    }
                  ...
                  map.on("dblclick", ROADS_TALUKS_FILL_LAYER_ID, (e) => {
                    ...
                    const feature = e.features?.[0];
                    const talukName =
                      (feature?.properties?.KGISTalukName as string | undefined)?.trim() ??
                      selectedRoadsTalukNameRef.current;
                    const talukGeometry =
                      feature?.geometry ?? selectedRoadsTalukGeometryRef.current;
                    const districtName = selectedRoadsDistrictNameRef.current;
                  ...
                  map.on("mousemove", ROADS_HOBLIES_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredRoadsHobliId !== null && hoveredRoadsHobliId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_HOBLIES_SOURCE_ID, id: hoveredRoadsHobliId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", ROADS_HOBLIES_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [ROADS_VILLAGES_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (selectedRoadsHobliIdRef.current === feature.id) {
                      ...
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_HOBLIES_SOURCE_ID, id: feature.id },
                        { selected: false },
                      );
                      selectedRoadsHobliIdRef.current = null;
                      selectedRoadsHobliGeometryRef.current = null;
                      clearRoadsVillages(map);
                      if (isRoadsHighwaysTalukScoped())
                    ...
                    if (selectedRoadsHobliIdRef.current !== null) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_HOBLIES_SOURCE_ID, id: selectedRoadsHobliIdRef.current },
                        { selected: false },
                      );
                    }
                  ...
                  map.on("dblclick", ROADS_HOBLIES_FILL_LAYER_ID, (e) => {
                    ...
                    const feature = e.features?.[0];
                    const hobliGeometry =
                      feature?.geometry ?? selectedRoadsHobliGeometryRef.current;
                    const districtName = selectedRoadsDistrictNameRef.current;
                    const talukName = selectedRoadsTalukNameRef.current;
                  ...
                  map.on("mousemove", ROADS_VILLAGES_FILL_LAYER_ID, (e) => {
                    ...
                    if (hoveredRoadsVillageId !== null && hoveredRoadsVillageId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_VILLAGES_SOURCE_ID, id: hoveredRoadsVillageId },
                        { hover: false },
                      );
                    }
                  ...
                  map.on("click", ROADS_VILLAGES_FILL_LAYER_ID, (e) => {
                    ...
                    if (selectedRoadsVillageIdRef.current !== null) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_VILLAGES_SOURCE_ID, id: selectedRoadsVillageIdRef.current },
                        { selected: false },
                      );
                    }
                    ...
                    if (!isSame) {
                      setFeatureStateSafe(
                        map,
                        { source: ROADS_VILLAGES_SOURCE_ID, id: feature.id },
                        { selected: true },
                      );
                    }
                    roadsSelectionGenerationRef.current++;
                    ...
                    if (isRoadsHighwaysTalukScoped())
                      applyRoadsBoundaryFilter(map, selectedRoadsHobliGeometryRef.current);
                  });
                  ...
                  map.on("click", STATE_DISTRICTS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [DISTRICT_TALUKS_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    const normalizedDistrictName = districtName?.trim().toLowerCase();
                    const taluksAlreadyLoaded =
                      loadedTaluksDistrictRef.current === normalizedDistrictName;

                    console.log(
                      `District: ${districtName}, Already loaded: ${taluksAlreadyLoaded}, Selected ID: ${selectedDistrictIdRef.current}, Feature ID: ${feature.id}`,
                    );
                  ...
                  map.on("mousemove", STATE_POLICE_POINT_LAYER_ID, (e) => {
                    ...
                    if (hoveredPolicePointId !== null && hoveredPolicePointId !== feature.id) {
                      setFeatureStateSafe(
                        map,
                        { source: STATE_POLICE_SOURCE_ID, id: hoveredPolicePointId },
                        { hover: false },
                      );
                    }
                    ...
                    setFeatureStateSafe(
                      map,
                      { source: STATE_POLICE_SOURCE_ID, id: feature.id },
                      { hover: true },
                    );
                    if (!drawingToolRef.current) map.getCanvas().style.cursor = "pointer";
                  });
                  ...
                  map.on("click", DISTRICT_TALUKS_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [TALUK_HOBLIES_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (selectedTalukIdRef.current !== null) {
                      ...
                      console.log(
                        `[Taluk Click] Feature properties:`,
                        JSON.stringify(feature.properties, null, 2),
                      );
                      console.log(
                        `[Taluk Click] Extracted talukName="${talukName}" from properties`,
                      );
                      ...
                      if (talukName !== "Unknown Taluk" && districtName && stateName) {
                        console.log(
                          `[Taluk Hoblies] Requesting hoblies for taluk="${talukName}", district="${districtName}", state="${stateName}"`,
                        );
                  ...
                  map.on("click", TALUK_HOBLIES_FILL_LAYER_ID, (e) => {
                    ...
                    if (
                      ...
                      queryRenderedFeaturesSafe(map, e.point, {
                        layers: [HOBLI_VILLAGES_FILL_LAYER_ID],
                      }).length > 0
                    ) {
                      return;
                    }
                    ...
                    if (selectedHobliIdRef.current !== null) {
                      ...
                      console.log(
                        `[Hobli Click] Feature properties:`,
                        JSON.stringify(feature.properties, null, 2),
                      );
                      ...
                      if (hobliName !== "Unknown Hobli" && talukName && districtName && stateName) {
                        console.log(
                          `[Hobli Villages] Requesting villages for hobli="${hobliName}", taluk="${talukName}", district="${districtName}", state="${stateName}"`,
                        );
                  ...
                  map.on("click", HOBLI_VILLAGES_FILL_LAYER_ID, (e) => {
                    ...
                    if (selectedVillageIdRef.current !== null) {
                      ...
                      console.log(
                        `[Village Click] Feature properties:`,
                        JSON.stringify(feature.properties, null, 2),
                      );
                      ...
                      if (
                        ...
                        console.log(
                          `[Village Cadastrals] Requesting cadastrals for village="${villageName}", hobli="${hobliName}", taluk="${talukName}", district="${districtName}", state="${stateName}"`,
                        );
                        void loadVillageCadastrals(
                          map,
                          villageName,
          ...
          if (event.key === "Escape") {
            ...
            if (event.key === "Enter") {
              ...
              if (session.points.length === 0) cancelDrawing(map);
                ...
                publishAOIData(map, draftPolygonFor(session.tool, session, null), [
                  ...session.points,
                ]);
              }
            }
  ...
  const processNavigationFix = async (
    ...
    const loadKarnatakaStateFromMinIO = async (map: MapLibreMap) => {
      try {
        ...
        console.log(
          `Successfully auto-loaded Karnataka State boundary with ${geojson.features.length} feature(s)`,
        );
        applyBoundaryLayerVisibility(map);
      } catch (error) {
        ...
        const coordinates = coordsText
          ...
          .filter(
            (coord): coord is [number, number] =>
              !isNaN(coord[0] ?? NaN) && !isNaN(coord[1] ?? NaN),
          );
    ...
    const loadBengaluruFileFeatures = async (key: string): Promise<GeoJSON.Feature[]> => {
      const response = await fetch(
        `/api/datasets/bengaluru-boundary-file?key=${encodeURIComponent(key)}`,
      );
      if (!response.ok) {
      ...
      try {
        ...
        if (wardQuery) {
          ...
          if (matched.length > 0) {
            ...
            const wardName = (matched[0]?.properties?.name as string | undefined) ?? wardQuery;
...
<kml xmlns="http://www.opengis.net/kml/2.2">
  <Document>
    ...
    useEffect(() => {
      ...
      const getBasemapLayerIds = (): string[] => {
        ...
        if (layer === "terrain") {
          ...
          return ["local-terrain-background", "local-dem-color-layer", "local-dem-hillshade"];
        }
        return [];
      };
    ...
    return (
      <div className="explore-map-root relative h-full w-full">
      ...
      `}</style>
        ...
        {!isLoading && !loadError && (
          <LayersControl
            ...
            isExpanded={activeMapPanel === "layers"}
            onToggle={(isExpanded) => setActiveMapPanel(isExpanded ? "layers" : "none")}
            showBadgeIcon={false}
        ...
        {weatherMode === "fire" &&
          ...
          (() => {
            ...
            if (containerWidth)
              left = Math.max(8, Math.min(left, containerWidth - POPUP_WIDTH - 8));
            if (containerHeight)
              top = Math.max(8, Math.min(top, containerHeight - POPUP_HEIGHT - 8));
                ...
                <dl className="mt-1.5 space-y-1 text-xs">
                  <div className="flex justify-between gap-2">
                    ...
                    <dd className="font-medium text-slate-900">
                      {formatIstTime(detection.acquiredAt)}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-2">
        ...
        {/* IMD district warning click popup - real source fields only (severity,
          message/impact/action, issued/valid times); never a fabricated summary. */}
        {showImdWarnings &&
          selectedImdWarning &&
          (() => {
            ...
            const containerWidth = containerRef.current?.clientWidth ?? 0;
            const containerHeight = containerRef.current?.clientHeight ?? 0;
            const w = selectedImdWarning.properties;
            const POPUP_HEIGHT = 120 + (w.description ? 30 : 0) + (w.impact ? 24 : 0) + (w.action ? 24 : 0);

            let left = selectedImdWarning.screenX + OFFSET;
            let top = selectedImdWarning.screenY - OFFSET - POPUP_HEIGHT;
            if (containerWidth && left + POPUP_WIDTH > containerWidth - 8) {
              left = selectedImdWarning.screenX - OFFSET - POPUP_WIDTH;
            }
            if (top < 8) top = selectedImdWarning.screenY + OFFSET;
            ...
            return (
              ...
              >
                <div className="flex items-start justify-between gap-2">
                  ...
                  <button
                    type="button"
                    onClick={() => setSelectedImdWarning(null)}
                    aria-label="Close"
                    className="-mr-1 -mt-1 rounded-full p-1 text-slate-400 hover:bg-slate-100 hover:text-slate-600"
                  >
                    ×
                ...
                <p className="mt-1 text-sm font-semibold text-slate-900">
                  {w.areaName ?? "Unknown district"}
                  {w.state ? `, ${w.state}` : ""}
                </p>
                {w.description && <p className="mt-1 text-xs text-slate-700">{w.description}</p>}
                <dl className="mt-1.5 space-y-1 text-xs">
                  <div className="flex justify-between gap-2">
                    <dt className="text-slate-500">Severity</dt>
                    <dd
                      className={`font-medium ${w.severity ? severityColor[w.severity] : "text-slate-900"}`}
                    >
                      {w.severity ?? "Unknown"}
                    </dd>
                  </div>
                  {w.issuedAt && (
                    <div className="flex justify-between gap-2">
                      ...
                      <dd className="font-medium text-slate-900">{formatIstTime(w.issuedAt)}</dd>
                    </div>
                  )}
                  {w.validUntil && (
                    <div className="flex justify-between gap-2">
                      <dt className="text-slate-500">Valid until</dt>
                      <dd className="font-medium text-slate-900">{formatIstTime(w.validUntil)}</dd>
                    </div>
                  )}
                  {w.impact && (
                    <div className="flex justify-between gap-2">
                      <dt className="text-slate-500">Impact</dt>
                      <dd className="text-right font-medium text-slate-900">{w.impact}</dd>
                    </div>
                  )}
                  {w.action && (
                    <div className="flex justify-between gap-2">
                      <dt className="text-slate-500">Action</dt>
                      <dd className="text-right font-medium text-slate-900">{w.action}</dd>
                    </div>
                  )}
                  <div className="flex justify-between gap-2">
                    <dt className="text-slate-500">Source</dt>
                    <dd className="font-medium text-slate-900">India Meteorological Department</dd>
                  </div>
                </dl>
              </div>
            );
                ...
                <div className="max-h-[calc(100vh-7.5rem)] overflow-y-auto overscroll-contain p-4">
                  ...
                  <div className="mt-4">
                    ...
                    {weatherMode === "clouds" && cloudProduct === "observed" && (
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="text-right font-medium text-slate-900">
                            Asia-Pacific, Americas, Atlantic
                          </span>
                        </div>
                        <div className="flex items-start justify-between gap-3">
                        ...
                        <div className="flex items-center gap-2 text-xs">
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 ${
                              himawariStatus === "ready"
                                ? "bg-green-100 text-green-700"
                                : "bg-gray-100 text-gray-500"
                            }`}
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${himawariStatus === "ready" ? "bg-green-500" : "bg-gray-400"}`}
                            />
                            Himawari
                          </span>
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 ${
                              goesWestStatus === "ready"
                                ? "bg-green-100 text-green-700"
                                : "bg-gray-100 text-gray-500"
                            }`}
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${goesWestStatus === "ready" ? "bg-green-500" : "bg-gray-400"}`}
                            />
                            GOES-West
                          </span>
                          <span
                            className={`inline-flex items-center gap-1 rounded-full px-2 py-0.5 ${
                              goesEastStatus === "ready"
                                ? "bg-green-100 text-green-700"
                                : "bg-gray-100 text-gray-500"
                            }`}
                          >
                            <span
                              className={`h-1.5 w-1.5 rounded-full ${goesEastStatus === "ready" ? "bg-green-500" : "bg-gray-400"}`}
                            />
                            GOES-East
                          </span>
                        </div>
                        {himawariStatus === "ready" && himawariFrames.length > 0 && (
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-slate-400">
                              {himawariFrames.length} frames · every {HIMAWARI_CADENCE_MINUTES} min
                        ...
                        <p className="text-[11px] text-slate-400">
                          Multi-geostationary IR composite (Band 13, 10.3 µm). Himawari covers
                          Asia-Pacific, GOES-West covers Americas/Pacific, GOES-East covers
                          Americas/Atlantic. Areas outside those satellite disks are now backfilled
                          with a global NOAA GFS cloud backdrop so the map stays visually
                          continuous.
                        </p>
                      </div>
                    )}
                    ...
                    {weatherMode === "rain" && rainProduct === "observed" && (
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          <span className="text-slate-500">Observed</span>
                          <span className="text-right font-medium text-slate-900">
                            {imergFrames[imergFrameIndex]
                              ? formatIstTime(imergFrames[imergFrameIndex]!)
                              : "Resolving..."}
                          </span>
                        </div>
                          ...
                          <div className="space-y-2">
                            <div className="flex items-center gap-2">
                              <button
                                ...
                                onClick={() => {
                                  setIsImergPlaying(false);
                                  setImergFrameIndex((i) =>
                                    i === 0 ? imergFrames.length - 1 : i - 1,
                                  );
                                }}
                                className="flex-1 rounded-xl border border-slate-200 bg-white px-2 py-1.5 text-xs font-medium text-slate-700 transition-colors hover:bg-slate-50"
                              >
                            ...
                            <p className="text-xs text-slate-400">
                              {imergFrames.length} frames · every {IMERG_CADENCE_MINUTES} min (real
                              timestamps only)
                            </p>
                          </div>
                        )}
                        {imergStatus === "loading" && (
                          <p className="text-xs text-slate-500">
                            Resolving latest GPM IMERG frames...
                          </p>
                        )}
                        ...
                        <p className="text-[11px] text-slate-400">
                          Real satellite-derived precipitation - typically runs 3-6h behind now
                          (genuine NRT processing latency, not a bug).
                        </p>
                      </div>
                    )}
                      ...
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        {weatherFieldStatus === "ready" && weatherFieldFrames.length > 0 && (
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-slate-400">
                              {weatherFieldIndex + 1} / {weatherFieldFrames.length} frames · GFS
                              forecast
                            </p>
                          </div>
                        )}
                      ...
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        <div>
                          ...
                          <div className="flex gap-1 rounded-xl bg-slate-100 p-1">
                            ...
                            {/* Air Temperature Forecast has no working data source (no public
                                BharatFS API/GRIB access exists - see FINAL_STAGE_3_REPORT.md).
                                Showing it as a normal, clickable mode would just lead users into
                                a dead end ("data access not configured"). Kept visible in dev only,
                                as a diagnostic of what's still pending real access. */}
                            {process.env.NODE_ENV !== "production" && (
                              ...
                              >
                                Air Temperature Forecast (dev)
                              </button>
                            )}
                          </div>
                        </div>

                        {temperatureProduct === "surface" || process.env.NODE_ENV === "production" ? (
                          <>
                            <div>
                              <p className="mb-1.5 text-xs font-semibold uppercase tracking-wide text-slate-500">
                                Platform
                              ...
                              <div className="rounded-xl border border-amber-200 bg-amber-50 p-2.5">
                                <div className="flex items-center justify-between gap-2">
                                  ...
                                  <span className="font-mono text-xs text-amber-700">
                                    {lstCoverage.coveragePercent}% ({lstCoverage.tilesValid}/
                                    {lstCoverage.tilesChecked} tiles)
                                  </span>
                                </div>
                                <p className="mt-1 text-[11px] text-amber-700">
                                  VIIRS is a polar-orbiting sensor with swath gaps. Blank regions =
                                  no satellite pass / cloud / invalid QA. Not interpolated or
                                  fabricated.
                                </p>
                              </div>
                            )}
                            ...
                            <div className="grid grid-cols-2 gap-2">
                              <WeatherMetaTile
                                label="Satellite"
                                value={lstResolved ? lstResolved.satelliteLabel : "Resolving..."}
                              />
                              <WeatherMetaTile
                                label="Platform"
                                value={lstResolved ? lstResolved.platform : "--"}
                              />
                              <WeatherMetaTile label="Instrument" value={VIIRS_LST_INSTRUMENT} />
                              <WeatherMetaTile
                                label="Product"
                                value={`${lstEffectiveDayNight === "day" ? "Day" : "Night"} pass`}
                              />
                              <WeatherMetaTile
                                label="Observation"
                                value={lstDate ?? "Loading..."}
                              />
                              <WeatherMetaTile label="Source" value="NASA GIBS" />
                            </div>
                            ...
                            {lstStatus === "loading" && (
                              <p className="text-xs text-slate-500">
                                Loading latest VIIRS surface temperature...
                              </p>
                            )}
                            {lstStatus === "unavailable" && (
                              <div className="space-y-2">
                                <p className="text-xs text-amber-700">
                                  Surface temperature temporarily unavailable.
                                </p>
                                <button
                                  type="button"
                                  onClick={() => {
                            ...
                            <details className="rounded-xl border border-slate-200 bg-slate-50 px-3 py-2.5">
                              <summary className="cursor-pointer text-xs font-medium text-slate-700">
                                Data notes and debug
                              </summary>
                              <div className="mt-3 space-y-3">
                                <p className="text-[11px] text-slate-500">
                                  Land Surface Temperature shows ground and surface heat, not air
                                  temperature. Cloud-covered or invalid areas stay transparent.
                                </p>
                                <p className="text-[11px] text-slate-500">{VIIRS_LST_ATTRIBUTION}</p>
                                <label className="flex cursor-pointer items-center gap-2">
                                  ...
                                  <span className="text-xs text-slate-600">
                                    Debug pixel inspector (dev only)
                                  </span>
                                </label>
                                {showLstDebug && lstResolved && (
                                  <div className="space-y-1 rounded border border-slate-200 bg-white p-2 font-mono text-[10px] text-slate-700">
                                    <div>
                                      <strong>Layer:</strong> {lstResolved.layer}
                                    </div>
                                    <div>
                                      <strong>Platform:</strong> {lstResolved.platform}
                                    </div>
                                    <div>
                                      <strong>Day/Night:</strong> {lstResolved.dayNight}
                                    </div>
                                    <div>
                                      <strong>Date:</strong> {lstResolved.date}
                                    </div>
                                    <div>
                                      <strong>TileMatrixSet:</strong> {VIIRS_LST_TILE_MATRIX_SET}
                                    </div>
                                    <div>
                                      <strong>Tile Size:</strong> {VIIRS_LST_TILE_SIZE}x{VIIRS_LST_TILE_SIZE}
                                    </div>
                                    <div>
                                      <strong>Native Max Zoom:</strong> {VIIRS_LST_NATIVE_MAX_ZOOM}
                                    </div>
                                    <div>
                                      <strong>Format:</strong> {VIIRS_LST_FORMAT}
                                    </div>
                                    <div>
                                      <strong>Nominal Resolution:</strong> {VIIRS_LST_NOMINAL_RESOLUTION_M} m
                                    </div>
                                    <div>
                                      <strong>Projection:</strong> Web Mercator (EPSG:3857)
                                    </div>
                                    {lstCoverage && (
                                      <>
                                        <hr className="my-1 border-slate-200" />
                                        <div>
                                          <strong>Coverage:</strong> {lstCoverage.coveragePercent}% over India
                                        </div>
                                        <div>
                                          <strong>Valid tiles:</strong> {lstCoverage.tilesValid}/{lstCoverage.tilesChecked}
                                        </div>
                                      </>
                                    )}
                                  </div>
                          ...
                          <div className="space-y-2">
                            ...
                            <p className="rounded-xl bg-amber-50 px-3 py-2 text-xs text-amber-800">
                              BharatFS data access not configured. No official machine-readable
                              BharatFS API/GRIB distribution was found during research - see the
                              report for what access would be needed. Surface Temperature (NASA
                              VIIRS) remains fully available above.
                            </p>
                          </div>
                        )}
                      </div>
                    ...
                    {weatherMode === "satellite" && (
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="text-right font-medium text-slate-900">
                            {satelliteResolved
                              ? satelliteResolved.product.satelliteLabel
                              : "Resolving..."}
                          </span>
                        </div>
                        <div className="flex items-start justify-between gap-3">
                        ...
                        {satelliteStatus === "ready" && satelliteAvailableDates.length > 0 && (
                          <div className="space-y-2">
                            ...
                            <p className="text-xs text-slate-400">
                              ...
                              {satelliteAvailableDates.length === 1 ? "" : "s"} available · Status:
                              Latest available
                            </p>
                          </div>
                        )}
                        {satelliteStatus === "loading" && (
                          <p className="text-xs text-slate-500">
                            Resolving latest NASA GIBS imagery...
                          </p>
                        )}
                        {satelliteStatus === "unavailable" && (
                          <div className="space-y-2">
                            <p className="text-xs text-amber-700">
                              Satellite imagery is temporarily unavailable (NOAA-21/NOAA-20/SNPP all
                              unreachable).
                            </p>
                            <button
                              type="button"
                        ...
                        <p className="text-[11px] text-slate-400">
                          VIIRS is polar-orbiting: it images the Earth in swaths through the day,
                          not all at once. Black areas are regions today&apos;s pass hasn&apos;t
                          covered yet (or cloud/no-data) - not a rendering error.
                        </p>
                      </div>
                    )}
                    ...
                    {weatherMode === "vegetation" && (
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        {ndviStatus === "loading" && (
                          <p className="text-xs text-slate-500">
                            Resolving latest NDVI composite...
                          </p>
                        )}
                        {ndviStatus === "unavailable" && (
                          <div className="space-y-2">
                            <p className="text-xs text-amber-700">
                              Vegetation imagery is temporarily unavailable (no recent composite
                              reachable).
                            </p>
                            <button
                              type="button"
                    ...
                    {weatherMode === "fire" && (
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        {fireStatus === "ready" && fireDetections.length === 0 && (
                          <p className="text-xs text-slate-500">
                            No active fires detected near this view.
                          </p>
                        )}
                        <p className="text-[11px] text-slate-400">
                  ...
                  <div className="mt-4 border-t border-slate-200/80 pt-4">
                    <div className="mb-2 flex items-center gap-2">
                      <CloudSun className="h-4 w-4 text-atlas-cobalt" aria-hidden />
                      <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-slate-400">
                        Click Map · Details
                      </p>
                    </div>
                    <p className="text-xs leading-relaxed text-slate-500">
                      Click anywhere on the map to view current weather, air quality and forecast.
                    </p>
                  </div>
                  ...
                  <div className="mt-4 border-t border-slate-200/80 pt-4">
                    <div className="flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-900">IMD Warnings</p>
                        <p className="text-xs text-slate-500">District nowcast, official IMD</p>
                      </div>
                      <label
                        className={`flex cursor-pointer items-center gap-2 text-sm text-slate-800 ${
                          weatherDetailsActive ? "" : "cursor-not-allowed opacity-40"
                        }`}
                      >
                        <input
                          type="checkbox"
                          className="accent-atlas-cobalt"
                          checked={showImdWarnings}
                          disabled={!weatherDetailsActive}
                          onChange={(event) => setShowImdWarnings(event.target.checked)}
                        />
                        <span className="font-medium">On</span>
                      </label>
                    </div>
                    {showImdWarnings && imdWarningsStatus === "loading" && (
                      <p className="mt-3 text-xs text-slate-500">Loading IMD district warnings...</p>
                    )}
                    {showImdWarnings && imdWarningsStatus === "ready" && (
                      <p className="mt-3 text-xs text-slate-400">
                        India Meteorological Department · short-range district nowcast. Tap a
                        highlighted district for details.
                      </p>
                    )}
                    {showImdWarnings && imdWarningsStatus === "unavailable" && (
                      <p className="mt-3 text-xs text-amber-700">
                        IMD warnings are temporarily unavailable. Everything else keeps working.
                      </p>
                    )}
                  </div>
                      ...
                      <div className="mt-3 space-y-3 text-sm text-slate-700">
                        ...
                        <div className="flex items-start justify-between gap-3">
                          ...
                          <span className="text-right font-medium text-slate-900">
                            {windFrames[activeWindFrameIndex]
                              ? formatIstShortDateTime(
                                  windFrames[activeWindFrameIndex]!.forecastTime,
                                )
                              : "Loading..."}
                          </span>
                        </div>
                    ...
                    <div className="mt-4 border-t border-slate-200/80 pt-4">
                      <div className="flex items-center justify-between gap-3">
                        <div>
                          ...
                          <p className="text-xs text-slate-500">
                            Modeled US AQI surface + official CPCB stations
                          </p>
                        </div>
                      </div>
        ...
        {isWindEnabled && windFrames.length >= 3 && !isLoading && !loadError && (
          <div className="bg-white/92 absolute bottom-6 left-1/2 z-20 w-[min(34rem,calc(100%-2rem))] -translate-x-1/2 rounded-3xl border border-white/70 p-4 shadow-2xl backdrop-blur-sm">
                ...
                <button
                  ...
                  className={`rounded-2xl border px-3 py-2 text-left transition-colors ${
                    index === activeWindFrameIndex
                      ? "bg-atlas-cobalt/10 border-atlas-cobalt"
                      : "border-slate-200 bg-white hover:bg-slate-50"
                  }`}
                >
                  <p className="text-xs font-semibold text-slate-900">+{frame.forecastHour}h</p>
