...
export function buildWorkflowTimeline(opts: BuildTimelineOptions): gsap.core.Timeline {
  ...
  for (const stage of WORKFLOW_STAGES) {
    ...
    // A tiny padding tween gives every sub a non-zero duration. GSAP does not fire
    // onStart (or position callbacks) of zero-duration children when a parent is driven
    // by seek/progress (as the deterministic tests do); 1ms is imperceptible in real
    // playback while making stage/callback timing identical in every render mode.
    sub.to({ t: 0 }, { t: 1, duration: 0.001, ease: "none" });
