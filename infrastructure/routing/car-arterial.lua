-- Car profile - based on OSRM's bundled /opt/car.lua (v6.0.0), with one change: residential/
-- living_street/unclassified/service roads are given lower assumed speeds than upstream's
-- defaults, so OSRM's duration-based routing model treats them as costing more time. Upstream
-- OSRM has no live-traffic or learned road-preference data (unlike Google), so on a dense grid
-- like central Bengaluru it can pick a zigzag through residential streets that's marginally
-- shorter on paper over a longer stretch of arterial road that any real driver (and Google)
-- would actually prefer. Lowering these speeds nudges the router toward arterials without
-- blocking residential roads outright - destinations still need their final-approach street.
-- See infrastructure/routing/build.sh (build_profile's optional 3rd arg) for how this gets
-- used instead of the bundled default for the driving profile only (walking/cycling are
-- unaffected - a residential street is exactly where you want a pedestrian/cyclist route to
-- go, unlike a car route trying to reach a highway).
  ...
  return {
    ...
    speeds = Sequence {
      highway = {
        ...
        tertiary        = 40,
        tertiary_link   = 20,
        -- Lowered from upstream's 25/25/10/15 (see the file-header comment above) - biases
        -- the driving profile toward tertiary-and-up roads without disabling these outright.
        unclassified    = 20,
        residential     = 18,
        living_street   = 8,
        service         = 10
